...
**Every model makes fewer rules errors with the skill and engine than without.**
306 scored cells · 4 skills × 3 conditions × 3 models · deterministic scorer, no human
judgement. The metric is **atomic error rate** — share of *verifiable units* (class, each
ability, AC, HP, each spell, each factual claim…) that are wrong. Lower is better.

**The three conditions:**
- `bare` — model answers from training memory alone; no context, no tools
- `grounding-only` — grounding rule injected ("don't trust training data") but no catalog access
- `skill-engine` — full skill protocol + model calls `node engine/cli.mjs` and interprets the output

**Key finding — grounding alone is not enough.** Telling a model "don't trust your training
data, read from the catalog" — without giving it catalog access — makes things *worse* in
every model tested. Only `skill-engine`, where the model actually runs the deterministic engine,
reliably cuts errors.

![Atomic error rate by model and condition across 4 skills](assets/bench-errors.svg)

**dnd-build (character creation) — consistent improvement across every model:**

| Model | Bare | + Grounding only | + Skill & engine | Reduction |
|-------|-----:|----------------:|-----------------:|----------:|
| Haiku 4.5 | 30.6% | 38.8% | **17.9%** | **−41%** |
| Sonnet 5 | 26.1% | 32.4% | **19.8%** | **−24%** |
| Opus 4.8 | 29.0% | 27.5% | **23.2%** | **−20%** |

From memory, models invent ~1 in 3 verifiable facts — wrong HP, fabricated feats, spells from
the wrong edition, changed ability scores. The dominant error category is **`invented-entity`**
(342 events), not arithmetic: models hallucinate D&D content wholesale. `skill-engine` replaces
invention with catalog lookup. The remaining errors are transcription slips, not hallucinations.

**Highlights across other skills:**
- **dnd-check / Opus / skill-engine:** 0 % atomic error — perfect sheet auditing from 60 % bare.
- **dnd-lookup / Haiku / skill-engine:** −59 % error rate (40.9 % → 16.7 %).

![Error taxonomy — bare condition: invented-entity leads at 342 events, lookup-omission 146, wrong-count 100](assets/bench-taxonomy.svg)

> Captures generated by Claude Code sub-agents, each running on its assigned model
> (Haiku 4.5 / Sonnet 5 / Opus 4.8) with condition-appropriate context. Results are indicative
> (n=1 per cell, not direct API calls). Scorer validated at 34/34 oracle tasks, zero errors
> ([self-check](benchmarks/reports/oracle-selfcheck.md)) ·
> [Full report →](benchmarks/reports/full-ablation-v1.md)

```bash
npm run bench:oracle                       # deterministic self-consistency (offline, 34/34)
npm run bench:replay -- --conditions bare,grounding-only,skill-engine   # re-score all 306 captures
node benchmarks/runner.mjs --backend live --conditions bare,grounding-only,skill-engine \
  --models haiku,sonnet,opus --reps 3 --dry-run        # live API run (needs ANTHROPIC_API_KEY)
```
