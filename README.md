...
metrics; the full method is in [benchmarks/README.md](benchmarks/README.md).

**Full ablation — 4 skills × 3 conditions × 3 models, 306 scored cells**
([report](benchmarks/reports/full-ablation-v1.md)). `bare` = model uses only training memory;
`grounding-only` = grounding rules injected, no catalog; `skill-engine` = skill protocol + engine
(`node engine/cli.mjs`) interpreted by the model itself:

![Atomic error rate by model and condition across 4 skills and 3 conditions](assets/bench-errors.svg)

**dnd-build (character creation) — clearest ablation signal:**

| Model | Bare | + Grounding rule | + Skill & engine | Best Δ |
|-------|-----:|----------------:|-----------------:|-------:|
| Haiku 4.5 | 30.6% | 38.8% | **17.9%** | −41% |
| Sonnet 5 | 26.1% | 32.4% | **19.8%** | −24% |
| Opus 4.8 | 29.0% | 27.5% | **23.2%** | −20% |

From memory, models get roughly 1 in 3–4 verifiable character-sheet units wrong (wrong HP, missed
spell, wrong saving throws). Adding the grounding rule alone helps little — the model *knows*
it should look up the catalog but still cannot. Running the skill and engine drops errors by
20–41% depending on the model; the remaining errors are transcription slips from engine output,
not hallucinations.

![Error taxonomy across all conditions — top categories: invented-entity, lookup-omission, wrong-count](assets/bench-taxonomy.svg)

> **Honest limits:** the captures used for this ablation were generated by Claude Code sub-agents
> (each assigned the correct model via the Agent tool) acting as the model under test, **not** by
> calling the Anthropic API directly. This is a valid proxy — each sub-agent runs on its assigned
> model (Haiku 4.5 / Sonnet 5 / Opus 4.8) and sees only the condition-appropriate context — but
> results should be treated as indicative, n=1 per cell. The scorer itself is validated by a
> deterministic oracle run at 34/34 tasks with zero errors
