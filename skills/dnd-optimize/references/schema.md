<!-- GENERATED from rules/schema.md by scripts/build-adapters.mjs — do not edit. -->
...
`scripts/dnd.mjs`) applies the **same** rules and the **same** math as the engine. Catalog
...
```
{
  ...
  abilityScores { str, dex, con, int, wis, cha, note? },   // English keys, always
...
## Progression 1–20 (`data/progression.json`)

Level milestones for the 12 classes, extracted mechanically from each class page's
progression table in the rules base (`docs/html/classes/*.html`) by
`scripts/extract-progression.mjs` — never authored by hand. Per class:

```
{ "classes": { "<classId>": { "source": "docs/html/classes/<file>.html",
    "levels": [ { level, pb, features[ {name, ref?} ],
                  slots{ "1".."9": count }?,        // leveled spell slots
                  pact{ slots, level }?,            // warlock Pact Magic
                  cantripsKnown?, preparedKnown?,
                  counters{ <slug>: value }? } ×20 ] } } }
```

- `pb` is the proficiency bonus at that level; `features` are the class-feature **names**
  (with a `ref` when the table links one) — their mechanical effects are *not* expanded.
- Use it to answer, with provenance, "at which character level does class X get spell
  slots of level L / an Ability Score Improvement / feature F". The earliest level whose
  `slots`/`pact.level` reaches a spell's level is the earliest that class can prepare it.
- Subclass features by level live in `data/subclasses.json` (`effects[].level`), not here.
- Read it via `node scripts/dnd.mjs progression <classId>` — not from memory.
- **Scope**: milestones only. Sheet *construction* (the build graph, `options`/`build`)
  still covers level 1 only; anything else a level-up needs is "Manquant documentaire".

## Answers → model (the guided-build input)

`scripts/dnd.mjs` also accepts an **answers** object (the guided-build format) and turns it
into the model above via `resolver.toCharacterModel`. Keys are the graph/choice ids, and the
head ones are **English slugs**: `class`, `subclass`, `species`, `lineage`, `background`,
`method`, `abilityScores`, `nom`, `alignement`, `_id`, plus one key per choice id the resolver
reports as pending (e.g. `fighter-competences`, `fighter-style-de-combat`,
`soldier-caracteristiques`, `origine-langues-choix`). Ability keys are `str dex con int wis cha`.

Discover the exact pending ids and their legal values with `options` — never guess them, and
never infer a pending id from the pattern: the entity half is an English slug but the suffix is
not, so guessing gets it wrong. A key that matches nothing is reported as an unknown answer key
and ignored; a value that is not in the option set is refused outright rather than computed.
