...
export function selectedSources(catalog, answers) {
  ...
  if (answers.class) {
    out.push({ id: "origine-langues", kind: "background", label: "Origin Languages",
      ref: "https://media.dndbeyond.com/compendium-images/srd/5.2/SRD_CC_v5.2.1.pdf#page=20",
...
export function toCharacterModel(catalog, answers) {
  const sources = [];
  for (const s of selectedSources(catalog, answers)) {
    if (s.kind === "choice-effect") { sources.push({ id: s.id, catalogId: s.id, kind: "order", label: s.label, ref: s.ref, effects: s.effects }); continue; }
    const requirements = s.choices
      .filter((choice) => choice.kind !== "cantrip" && choice.kind !== "prepared")
      .map((choice) => ({
        type: "requiresChoice",
        kind: choice.kind,
    ...
    sources.push({ id: s.id, catalogId: s.id, kind: s.kind, label: s.label, ref: s.ref,
      effects: [
        ...s.effects.filter((e) => e.type === "grants" || e.type === "effect"),
        ...requirements,
      ] });
  }
  const choices = [];
  ...
  return {
    ...
    classId: answers.class || null,
    speciesId: answers.species || null,
    lineageId: answers.lineage || null,
    backgroundId: answers.background || null,
    ...
    abilityScores: answers.abilityScores || { str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10 },
    sources, choices,
...
function equipItemBase(catalog, itemId) {
  ...
  if (a) return a.category === "bouclier"
    ? { id: a.id, object: a.name, roleIds: ["shield"], roles: ["bouclier"], shield: true }
    : { id: a.id, object: a.name, roleIds: ["armor"], roles: ["armure"], armor: { base: a.baseAC, dexMax: a.dexMax } };
  const w = byId(eq.weapons, itemId);
  if (w) {
    const vers = (w.properties || []).find((p) => /polyvalente/i.test(p));
    const vd = vers && vers.match(/\(([^)]+)\)/);
    const wp = { damage: w.damage, type: w.damageType, ability: w.type === "distance" ? "dex" : "str" };
    if (vd) wp.versatile = vd[1];
    return { id: w.id, object: w.name, roleIds: ["weapon"], roles: ["arme"], damageType: w.damageType, weapon: wp };
  }
  const t = byId(eq.tools, itemId); if (t) return { id: t.id, object: t.name, roleIds: ["tool"], roles: ["outil"] };
  const p = byId(eq.packs, itemId); if (p) return { id: p.id, object: p.name, roleIds: ["pack"], roles: ["paquetage"] };
  const g = byId(eq.gear, itemId);
  if (g) {
    const focus = g.note && /focaliseur/i.test(g.note);
    return { id: g.id, object: g.name, roleIds: [focus ? "spellcasting-focus" : "gear"], roles: [focus ? "focaliseur d'incantation" : "objet"] };
  }
  return null;
}
